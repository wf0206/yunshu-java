...
public class EsGoodsSearchServiceImpl implements EsGoodsSearchService {
    ...
    public SearchPage<EsGoodsIndex> searchGoods(EsGoodsSearchDTO searchDTO, PageVO pageVo) {
        ...
        SearchHitSupport.searchPageFor(search, searchQuery.getPageable());
        SearchPage<EsGoodsIndex> searchHits = SearchHitSupport.searchPageFor(search, searchQuery.getPageable());
        String lang = request.getHeader("LANG");
        if(ObjectUtil.isEmpty(lang)){
            lang = "zh";
        }
        if(CollUtil.isNotEmpty(searchHits.getContent())){
            List<SearchHit<EsGoodsIndex>> searchHitList = searchHits.getContent();
            for (SearchHit<EsGoodsIndex> esGoodsIndexSearchHit : searchHitList) {
                EsGoodsIndex esGoodsIndex = esGoodsIndexSearchHit.getContent();
                setCategoryByLang(esGoodsIndex);
            }
        }
        return searchHits;
    }

    private EsGoodsIndex setCategoryByLang( EsGoodsIndex esGoodsIndex){
        String lang = request.getHeader("LANG");
        if ("en".equals(lang) && ObjectUtil.isNotEmpty(esGoodsIndex.getGoodsUkName())){
            esGoodsIndex.setGoodsName(esGoodsIndex.getGoodsUkName());
            esGoodsIndex.setMobileIntro(esGoodsIndex.getMobileUkIntro());
        }
        if ("zh-Hant".equals(lang)  && ObjectUtil.isNotEmpty(esGoodsIndex.getGoodsMyName())){
            esGoodsIndex.setGoodsName(esGoodsIndex.getGoodsMyName());
            esGoodsIndex.setMobileIntro(esGoodsIndex.getMobileMyIntro());
        }
        return esGoodsIndex;
    }
    ...
    private List<SelectorOptions> convertCategoryOptions(List<? extends Terms.Bucket> categoryBuckets) {
        ...
        Map<String, Category> categoryMap = list.stream().collect(Collectors.toMap(Category::getName, x -> x,(z,y)->z));
        String lang = request.getHeader("LANG");
            ...
            if (split.length == nameSplit.length) {
                for (int i = 0; i < split.length; i++) {
                    ...
                    if("zh-Hant".equals(lang)){
                        so.setName(categoryMap.get(s).getMyName());
                    }
                    else if("en".equals(lang)){
                        so.setName(categoryMap.get(s).getUkName());
                    }
                    else {
