...
public class OrderServiceImpl extends ServiceImpl<OrderMapper, Order> implements OrderService {
    ...
    public IPage<OrderSimpleVO> queryByParams(OrderSearchParams orderSearchParams) {
        ...
        IPage<OrderSimpleVO> iPage = this.baseMapper.queryByParams(PageUtil.initPage(orderSearchParams), queryWrapper);
        String lang = request.getHeader("LANG");
        for (OrderSimpleVO record : iPage.getRecords()) {
            if(ObjectUtil.isNotEmpty(record.getGroupName())){
                List<GoodsSku> list = categoryService.list(Wrappers.<GoodsSku>lambdaQuery()
                        .eq(GoodsSku::getGoodsName,record.getGroupName()));
                Map<String, GoodsSku> categoryMap = list.stream().collect(Collectors.toMap(GoodsSku::getGoodsName, x -> x,(z, y)->z));
                if("zh-Hant".equals(lang) && ObjectUtil.isNotEmpty(categoryMap) && ObjectUtil.isNotEmpty(categoryMap.get(record.getGroupName()))){
                    record.setGroupName(categoryMap.get(record.getGroupName()).getGoodsMyName());
                }
                else if("en".equals(lang) && ObjectUtil.isNotEmpty(categoryMap) && ObjectUtil.isNotEmpty(categoryMap.get(record.getGroupName()))){
                    record.setGroupName(categoryMap.get(record.getGroupName()).getGoodsUkName());
                }
            }
