...
//        List<CategoryVO> categoryVOList = (List<CategoryVO>) cache.get(CachePrefix.CATEGORY.getPrefix() + lang);
...
//        }
        ...
        for (Category category : list) {
            if ("0".equals(category.getParentId())) {
                category = setCategoryByLang(category);
        ...
        if (!categoryVOList.isEmpty()) {
            cache.put(CachePrefix.CATEGORY.getPrefix() + lang, categoryVOList);
    ...
    public List<CategoryVO> listAllChildren() {
        ...
        List<CategoryVO> categoryVOList1 = (List<CategoryVO>) cache.get(CachePrefix.CATEGORY.getPrefix() + "all");
        ...
        categoryVOList.sort(Comparator.comparing(Category::getSortOrder));
        cache.put(CachePrefix.CATEGORY.getPrefix() + "all",categoryVOList);
        return categoryVOList;
    }
    ...
    private void removeCache() {
        ...
        cache.remove(CachePrefix.CATEGORY.getPrefix() + "zh-Hans");
        cache.remove(CachePrefix.CATEGORY.getPrefix() + "en");
        cache.remove(CachePrefix.CATEGORY.getPrefix() + "zh-Hant");
        cache.remove(CachePrefix.CATEGORY.getPrefix() + "all");
    }

    private Category  setCategoryByLang(Category category){
        String lang = request.getHeader("LANG");
        if ("en".equals(lang)){
            category.setName(category.getUkName());
        }
        if ("zh-Hant".equals(lang)){
            category.setName(category.getMyName());
        }
        return category;
    }
}