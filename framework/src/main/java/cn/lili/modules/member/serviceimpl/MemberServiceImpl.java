...
public class MemberServiceImpl extends ServiceImpl<MemberMapper, Member> implements MemberService {
    ...
    public Token mobilePhoneLogin(String mobilePhone) {
        ...
        //如果手机号不存在则自动注册用户
        if (member == null) {
            member = new Member(mobilePhone, UuidUtils.getUUID(), mobilePhone,null);
            registerHandler(member);
        }
        loginBindUser(member);
        return memberTokenGenerate.createToken(member, false);
    ...
    @Override
    public Token register(String userName, String password, String mobilePhone, String gradeId) {
        //检测会员信息
        checkMember(userName, mobilePhone);
        //设置会员信息
        Member member = new Member(userName, new BCryptPasswordEncoder().encode(password), mobilePhone,gradeId);
        //注册成功后用户自动登录
        registerHandler(member);
    ...
    public Member addMember(MemberAddDTO memberAddDTO) {
        ...
        //添加会员
        Member member = new Member(memberAddDTO.getUsername(), new BCryptPasswordEncoder().encode(memberAddDTO.getPassword()), memberAddDTO.getMobile(),null);
        registerHandler(member);
        return member;
    }
